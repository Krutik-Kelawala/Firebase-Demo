...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> loginUser() async {
    try {
      ...
      CommonWidgets.printFunction("credential $credential");
      if (context.mounted) {
        CommonWidgets.commonSuccessToast("Login Successfully!", context);
      }
    } on FirebaseAuthException catch (e) {
      CommonWidgets.printFunction("firebase error ${e.toString()}");
      if (e.code == 'user-not-found') {
        CommonWidgets.printFunction('No user found for that email. ${e.toString()}');

        if (context.mounted) {
          CommonWidgets.commonErrorToast("No user found", context);
        }
      } else if (e.code == 'wrong-password') {
        CommonWidgets.printFunction('Wrong password provided for that user.${e.toString()}');
        if (context.mounted) {
          CommonWidgets.commonErrorToast("Wrong password", context);
        }
      } else if (e.code == "invalid-credential") {
        CommonWidgets.printFunction('invalid-credential for that user.${e.toString()}');
        if (context.mounted) {
          CommonWidgets.commonErrorToast("Invalid credential", context);
        }
      }
    } catch (e) {
      CommonWidgets.printFunction("error ${e.toString()}");
    }
  }
